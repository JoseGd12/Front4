...
interface InsumoEntregaResumen {
  id: number;
  nombre: string;
  cantidad: number;
  precio: number;
...
interface DetailPanelEntregaProps {
  insumos: InsumoEntregaResumen[];
  cantidadTotal: number;
  valorTotal: number;
  onRemoveInsumo?: (insumoId: number) => void;
  getTarjetaInput?: (
    insumo: { id: number; cantidad: number },
    campo: "cantidad"
  ) => string;
...
export function DetailPanelEntrega({
  insumos,
  cantidadTotal,
  valorTotal,
  onRemoveInsumo,
  getTarjetaInput,
  onTarjetaInputChange,
}: DetailPanelEntregaProps) {
  const tieneItems = insumos.length > 0;

  return (
    <div className="elegante-card h-full min-h-0 overflow-hidden flex flex-col">
      <div className="sticky top-0 z-10 px-5 py-3 border-b border-gray-dark bg-gradient-to-r from-green-500/10 to-gray-darkest">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-2">
              {insumos.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="bg-gray-darker rounded-lg px-3 py-2.5 border-l-2 border-green-500/20 venta-item-enter"
                  >
                    <div className="flex items-center gap-3 flex-nowrap min-w-0">
                      ...
                      <div className="flex flex-col gap-0.5 shrink-0">
                        ...
                        <span className="text-green-400 font-semibold text-xs tabular-nums leading-7">
                          ${formatCurrency(p.cantidad * p.precio)}
                        </span>
                      </div>

                      <button
                        onClick={() => onRemoveInsumo?.(p.id)}
        ...
        {!tieneItems && (
          <div className="px-5 py-8 text-center">
            ...
            <p className="text-sm text-gray-lightest">
              Agrega insumos para ver el resumen de la entrega
            </p>
          </div>
        )}
        ...
        >
          <span className="text-white-primary font-semibold text-sm">Cantidad Total</span>
        ...
        >
          <span className="text-white-primary font-semibold text-sm">Valor Unitario Total</span>
          <span className="text-white-primary font-bold text-lg tabular-nums">
      ...
      <div className="shrink-0 px-5 pt-3 pb-4 border-t border-gray-dark bg-gray-darkest/90">
        ...
        >
          <span className="text-white-primary font-bold text-base tracking-wide">TOTAL ENTREGA</span>
